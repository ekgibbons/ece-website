    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', function () {
            ...
            getGradeSelects().forEach(sel => {
                ...
                // Try to find the semester input by name, id, or data attribute
                let sem = '';
                // 1. Try form.elements
                if (form?.elements[course + 'sem']) {
                    sem = form.elements[course + 'sem'].value;
                } else {
                    // 2. Try getElementById
                    const semInput = document.getElementById(course + 'sem');
                    if (semInput) {
                        sem = semInput.value;
                    } else {
                        // 3. Try querySelector for input[data-course=...] and [data-semester]
                        const semField = document.querySelector(`input[data-course='${course}'][data-semester], input[name='${course}sem']`);
                        if (semField) {
                            sem = semField.value;
                        } else {
                            sem = 'Not taken';
